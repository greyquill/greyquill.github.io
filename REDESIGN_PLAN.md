...
- **2026-05-02** — **Phase 2 in progress.** Built `/products` index + four product pages: ClarityAI (with external CTAs to `clarity.greyquill.io`), GQData, GQ Agents (Champ-derived), GST Co-Pilot (IBM-GST-derived, vendor-neutral copy). Each follows the same template: hero with custom right-side visual, dilemma stats, 4 capabilities, 3 user roles, cross-links to sibling products, dark final CTA. Header gained a Products dropdown (CSS-only, hover + focus-within) listing all four with tier badges. Remaining Phase 2: `/partnerships`, plus keep-as-is page ports.
